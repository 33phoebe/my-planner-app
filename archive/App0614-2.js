...
// Enhanced TaskList with drag & drop zones
// Enhanced TaskList with undo support
const TaskList = () => {
    ...
    // Separate section for habits
    const unscheduledHabits = tasks.filter(t => !t.completed && !t.dueDate && t.isHabit);
...
const StatCard = ({ title, count, color = "gray" }) => {
    ...
    return (
        <div className="p-4 sm:p-6 bg-white rounded-2xl shadow-sm max-w-6xl mx-auto space-y-6">
            ...
            {/* Habits Section - Only show if there are unscheduled habits */}
            {unscheduledHabits.length > 0 && (
                <div className="space-y-3">
                    ...
                    >
                        <div className="space-y-2">
                            {unscheduledHabits.map(habit => <Task key={habit.id} task={habit}/>)}
                        </div>
                    </DroppableZone>
                </div>
            )}
...
// Improved MealPlanner with better text handling
const MealPlanner = ({ currentWeek }) => {
    ...
    const handleShoppingListChange = (value) => {
        updateWeeklySummary(weekKey, {...weeklyData, shoppingList: value});
    };
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="xl:col-span-2 bg-white rounded-2xl shadow-sm p-6">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full border-collapse">
                        <thead>
                            <tr>
                                <th className="text-left p-2 font-semibold text-gray-600 w-20">Day</th>
                                {mealTypes.map(mealType => (
                                    <th key={mealType} className="text-left p-2 font-semibold text-gray-600 min-w-32">{mealType}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {weekDays.map(day => (
                                <tr key={day.toString()} className="border-t border-gray-100">
                                    ...
                                    {mealTypes.map(mealType => (
                                        <td key={mealType} className="p-1">
                                            <textarea
                                                value={weeklyData.meals?.[format(day, 'yyyy-MM-dd')]?.[mealType] || ''}
                                                onChange={(e) => handleMealChange(day, mealType, e.target.value)}
                                                className="w-full p-2 border border-gray-200 bg-gray-50 rounded-lg text-xs focus:ring-1 focus:ring-teal-500 focus:border-teal-500 resize-none"
                                                rows="2"
            ...
            {/* Shopping List */}
            <div className="bg-orange-50 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center space-x-2">
                            <button 
                                ...
                                className="text-xs font-semibold text-orange-700 hover:text-orange-900 disabled:text-gray-400 px-2 py-1 rounded bg-white/50 hover:bg-white/70"
                            >
                                ✨ Generate
                            </button>
                            <button 
                                onClick={() => setIsEditingShoppingList(!isEditingShoppingList)} 
                                className="text-xs font-semibold text-orange-700 hover:text-orange-900 px-2 py-1 rounded bg-white/50 hover:bg-white/70"
                            >
                                {isEditingShoppingList ? 'Preview' : 'Edit'}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-4 h-80">
                    {isEditingShoppingList ? (
                        <textarea 
                            ...
                            className="w-full h-full p-3 border-0 bg-white rounded-lg focus:ring-2 focus:ring-orange-500 resize-none text-sm"
                            disabled={isGeneratingList}
                        />
                    ) : (
                        <div className="prose prose-sm max-w-none h-full overflow-y-auto bg-white rounded-lg p-3">
