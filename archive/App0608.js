...
const Task = ({ task }) => {
    ...
    return (
        <>
            <div ref={drag} onClick={() => setShowDetailModal(true)} className={`group relative p-1.5 rounded-lg shadow-sm mb-2 transition-all duration-200 border-l-4 cursor-pointer ${task.completed ? 'bg-green-100 text-gray-500 line-through border-green-400' : 'bg-white hover:bg-gray-50'} ${task.isHabit ? 'border-violet-400' : 'border-orange-400'} ${isDragging ? 'opacity-50 scale-105 shadow-lg' : 'opacity-100'}`}>
                <div className="flex items-start justify-between">
                    <div className="flex items-center flex-grow min-w-0">
                        <input type="checkbox" checked={task.completed} onChange={handleToggleComplete} onClick={(e) => e.stopPropagation()} className={`h-4 w-4 rounded-full border-gray-300 focus:ring-0 focus:ring-offset-0 transition-all ${task.isHabit ? 'text-violet-500' : 'text-orange-500'}`} />
                        <span className="ml-2 font-medium text-xs text-gray-800 line-clamp-2">{task.title}</span>
                    </div>
                    {task.isHabit && (
                        <button onClick={(e) => {e.stopPropagation(); setShowHabitManager(true)}} className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 p-1 rounded-full text-xs text-violet-600 font-semibold">...</button>
                    )}
                    {task.estimatedTime && <span className="ml-2 text-[10px] font-semibold text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded-full flex-shrink-0">{task.estimatedTime}m</span>}
                </div>
                {task.description && <p className="text-xs text-gray-500 mt-1 ml-6 truncate" title={task.description}>{task.description}</p>}
            </div>
             ...
             <div className="grid grid-cols-7 gap-1">
                {weeks.map((weekStart, i) => 
                    eachDayOfInterval({start: weekStart, end: endOfWeek(weekStart, {weekStartsOn: 1})}).map(day => {
                        ...
                        return (
                            <div key={day.toString()} onClick={() => handleDayClick(day)} className={`p-2 border border-gray-200 rounded-lg h-32 flex flex-col cursor-pointer transition-colors ${isCurrentMonth ? 'bg-white hover:bg-teal-50' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}>
                                ...
                                <div className="mt-1 space-y-1 overflow-y-auto text-xs">
                                    {tasksForDay.map(task => (
                                        <div key={task.id} className={`p-1 rounded truncate ${task.isHabit ? 'bg-violet-100 text-violet-800' : 'bg-orange-100 text-orange-800'}`}>{task.title}</div>
                                    ))}
                                </div>
                            </div>
                        )
