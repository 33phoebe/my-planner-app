...
// Enhanced Task component with context-aware sizing and custom drag preview
const Task = ({ task, compact = false, showDragHandle = true }) => {
    ...
    // Set custom drag preview to be empty so we can use our custom drag layer
    useEffect(() => {
        const img = new Image();
...
// Update your DayColumn component to use the new Task component properly
...
const TaskList = () => {
    ...
    const DroppableZone = ({ onDrop, children, isEmpty, emptyMessage, className = "" }) => {
        const [{ isOver }, drop] = useDrop(() => ({
            accept: ItemTypes.TASK,
            drop: onDrop,
            collect: (monitor) => ({
        ...
        return (
            ...
            >
                {isEmpty && isOver ? (
                    <div className="flex items-center justify-center h-24 text-teal-600 font-medium">
                        Drop task here
                    </div>
                ) : isEmpty ? (
    ...
    return (
        <div className="p-4 sm:p-6 bg-white rounded-2xl shadow-sm max-w-4xl mx-auto space-y-8">
            ...
            <CollapsibleSection title="Scheduled Tasks">
                <DroppableZone
                    onDrop={(item) => {
                        // Keep existing date or set to today if no date
            ...
            <CollapsibleSection title="Parking Lot" icon={<ParkingLotIcon/>}>
                ...
                <DroppableZone
                    onDrop={(item) => updateTask(item.id, { dueDate: null })}
                    isEmpty={parkingLotTasks.length === 0}
                    emptyMessage="Your parking lot is empty!"
            ...
            <CollapsibleSection title="Completed Tasks">
                <DroppableZone
                    onDrop={(item) => {
                        // Don't allow dropping into completed - this is just for visual consistency
                    }}
...
// Custom Drag Layer for visual preview
const CustomDragLayer = () => {
    ...
    return (
        <div className="fixed pointer-events-none z-50 top-0 left-0" style={{
            transform: `translate(${currentOffset.x}px, ${currentOffset.y}px)`,
        }}>
            <div className="bg-white border-2 border-teal-300 rounded-lg p-2 shadow-lg max-w-xs opacity-90">
                <div className="flex items-center">
                    <div className={`w-3 h-3 rounded-full mr-2 ${item.isHabit ? 'bg-violet-400' : 'bg-orange-400'}`}></div>
                    <span className="font-medium text-sm text-gray-800 truncate">{item.title}</span>
                </div>
                {item.estimatedTime && (
                    <span className="text-xs text-gray-500 ml-5">{item.estimatedTime}m</span>
                )}
            </div>
        </div>
    );
    ...
    const DroppableDayCell = ({ day, tasksForDay, isCurrentMonth }) => {
        ...
        return (
            <div 
                ...
                className={`p-2 border border-gray-200 rounded-lg h-32 flex flex-col cursor-pointer transition-all duration-200 ${
            ...
            >
                ...
                <div className="space-y-0.5 overflow-y-auto flex-1">
                    {tasksForDay.slice(0, 4).map(task => (
                        <Task key={task.id} task={task} compact={true} />
                    ))}
...
// Update your main App return statement to include the CustomDragLayer
